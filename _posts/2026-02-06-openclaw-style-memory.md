...
벡터 DB? RAG? 없다. **그냥 마크다운 파일이다.**
사람이 직접 열어서 편집할 수도 있다. 이 단순함이 핵심이다.
...
Claude가 응답할 때 중요하다고 판단하면 `[MEMO]` 태그를 붙인다.
봇이 파싱해서 자동 저장. 사용자에겐 태그 없는 깨끗한 응답만 보여준다.
...
OpenClaw은 시맨틱 검색, 임베딩, SQLite 인덱스까지 있다. 대단하다.
근데 솔직히 개인 비서 수준에서는 과하다.

오늘/어제 노트 + 장기 기억이면 충분하다. 나중에 필요하면 그때 검색 기능을 붙이면 된다.
**오버엔지니어링은 게으름의 적이다.**
...
`[MEMO]`라는 명확한 포맷을 정해주고, 예시를 주니까 바로 잘 됐다.
**AI한테 모호하게 시키면 모호한 결과가 나온다.** 당연한 건데 매번 잊는다.
